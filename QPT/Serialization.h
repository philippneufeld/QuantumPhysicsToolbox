...
template <>
struct SerializationTraitsHelper<std::string> {
  static const char* SerializeTrivial(const std::string& val) {
    return val.data();
  }
  static char* DeserializeTrivial(std::string& val) { return val.data(); }
  static void Prepare(std::string& val, std::size_t n) { val.resize(n); }
  static std::size_t GetSize(const std::string& val) { return val.size(); }
};
